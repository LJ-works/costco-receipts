...
export async function showPricingWarningUi(options: {
  ...
  const dealIndex = buildWarehouseDealIndex(options.warehouseDeals ?? []);
  ...
  function createRow(id: string): HTMLElement {
    ...
    const deals = dealIndex.get(id) ?? [];
    const active = deals.length > 0;
    ...
    offer.textContent = active
      ? deals.map((deal) => deal.offer.raw.displayText || deal.title).join(" | ")
      : "No current Warehouse Savings offer";
